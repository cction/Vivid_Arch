...
## 代理B（Grsai）与占位符交互经验
- **非破坏性占位符更新**：
  - 在生成任务（特别是长耗时任务）失败或超时后，**不要删除占位符**。
  - 应保留占位符并更新其状态（failed/timeout），同时将任务元数据（如 `taskId`）写入元素。
  - 这样用户既保留了上下文（位置、尺寸），又能通过 taskId 进行重试或排查，避免“白等一场”后一无所有。
- **SVG foreignObject 交互**：
  - 在 SVG `foreignObject` 内嵌入 HTML 交互元素（如按钮）时，必须在点击事件中调用 `e.stopPropagation()` 和 `e.preventDefault()`。
  - 否则点击事件会冒泡到外层 SVG/Canvas，触发画布的拖拽、框选或缩放逻辑，导致用户体验冲突。
- **任务ID 透出与闭环**：
  - 对于异步生成任务，服务端接口应设计为“即使超时/Pending 也返回 taskId”。
  - 前端拿到 taskId 后，即使当前轮询失败，也可在 UI 提供“单次查询（One-shot Query）”按钮，实现基于 taskId 的手动重试闭环。
