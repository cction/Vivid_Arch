...
- 修复合并图层时描边变细的问题，确保合并后的视觉效果与当前缩放倍率一致。
- 修复合并带圆角图片时可能因裁切坐标错误导致图片不可见的问题。
- 修复合并外部图片链接时因跨域导致加载失败的问题，增加自动转 Base64 处理。
- 将代理 B 的 Professional 调用模型调整为 nano-banana-pro-cl，保持占位图与分辨率逻辑与专业档一致。
