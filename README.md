# BananaPod｜图像生成与编辑工作台（v1.4.5）

轻量、可扩展的图像生成与编辑工作台，采用 PodUI 主题。统一第三方 API 为 OpenAI 风格接口（基于 `whatai.cc`），支持图层系统、裁剪与合并、双语界面、iPad/Apple Pencil 场景。

## 核心更新（v1.4.5）
- 优化：最近使用提示词列表的展示，字体与布局更清晰紧凑。
- 优化：工作台 PromptBar 与左侧栏、标题栏采用统一深色主题，整体观感更协调。
- 优化：工作台紧凑模式下的背景与搜索栏透明效果，阅读性更好。
...
- 当前版本：`1.4.5`
- 关键变更：
  - 最近使用提示词展示更紧凑，避免字体被裁切。
  - 工作台 PromptBar 与左右区块配色统一为深色主题。
  - 紧凑模式与搜索栏透明背景的视觉对比度与易读性优化。
