# BananaPod｜图像生成与编辑工作台（v1.4.4）

轻量、可扩展的图像生成与编辑工作台，采用 PodUI 主题。统一第三方 API 为 OpenAI 风格接口（基于 `whatai.cc`），支持图层系统、裁剪与合并、双语界面、iPad/Apple Pencil 场景。

## 核心更新（v1.4.4）
- 新增：最近使用提示词列表，可在快捷效果窗口一键复用常用文案。
- 优化：发送到 API 的图片自动限制在 2K 分辨率以内，提升兼容性与出图稳定性。
- 优化：所有用户可见错误信息统一脱敏，避免在界面中出现具体模型或服务名称。
...
- 当前版本：`1.4.3`
- 关键变更：
  - 天气预设改为标签显示在 PromptBar 输入框前缀，隐藏系统提示词。
  - 标签支持键盘删除与仅标签生成，输入体验更贴近编辑文本。
  - 工作台左侧预设缩略图保持彩色显示，选中状态更清晰。
