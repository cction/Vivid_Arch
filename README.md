# BananaPod｜图像生成与编辑工作台（v1.4.0）

轻量、可扩展的图像生成与编辑工作台，采用 PodUI 主题。统一第三方 API 为 OpenAI 风格接口（基于 `whatai.cc`），支持图层系统、裁剪与合并、双语界面、iPad/Apple Pencil 场景。

## 核心更新（v1.4.0）
- 工作台：新增香蕉按钮工作台视图，支持天气预设 + 网页 + Prompt 三栏布局。
- 网页联动：嵌入 VividAI 网页，复制提示词可同步到工作台底部 PromptBar。
- 体验：优化工作台整体视觉为更现代扁平风格，预设卡片与布局更紧凑。

## 核心更新（v1.2.8）
- 设置：在设置面板右侧新增“最近更新”区域，自动展示最近三个版本的主要改动。
- 布局：调整设置面板为左窄右宽布局，左侧设置更紧凑，右侧更新信息更清晰。
- 交互：优化 API 密钥输入与保存按钮排版，在窄屏下避免横向滚动，并统一按钮圆角与尺寸风格。
- 文档：简化 CHANGELOG 写法，只保留面向用户的更新描述，与“最近更新”实现自动同步。
...
- UI：PromptBar 模型展示名改为 `Standard_A/Professional_A` 与 `Standard_B/Professional_B`（请求仍使用原模型 id）
- 兼容：旧 `localStorage` 中的模型值自动回退归一化，避免升级后模型不可选导致报错
- 验证：通过 `npm run lint`、`npx tsc --noEmit`、`npm run build`
...
- 去除背景与装饰层，悬停轻微放大并提亮，按下轻微缩小
- `BananaSidebar.tsx` 直接渲染 `BananaIcon`；`podui.css` 简化交互样式（透明背景、轻交互）
- 文档与版本号统一到 `v1.1.7`
...
- 当前版本：`1.4.0`
- 关键变更：
  - 新增香蕉按钮工作台视图，整合天气预设与网页内容。
  - VividAI 网页复制提示词可一键同步到底部 PromptBar。
  - 工作台 UI 扁平化与布局优化，保证大屏工作区的视野与使用体验。
